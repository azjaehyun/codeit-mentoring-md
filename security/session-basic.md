...
## **0. 본격적으로 들어가기 전에: 핵심 용어부터 정리하기**

본문에 들어가기 전에 계속 반복해서 등장할 용어들을 먼저 짚고 넘어가겠습니다. 이 용어들의 뜻을 정확히 모르고 코드를 보면, 코드 자체는 어렵지 않은데도 "이게 왜 필요한 건지" 감이 잡히지 않는 경우가 많습니다. 반대로 용어의 의미만 확실히 잡아두면, 뒤에 나오는 코드들은 사실 이 개념들을 그대로 코드로 옮겨놓은 것에 불과하다는 걸 알게 될 것입니다.

### **인증 (Authentication)**

인증은 쉽게 말해 "당신이 누구인지 확인하는 절차"입니다. 로그인 화면에서 이메일과 비밀번호를 입력하고 로그인 버튼을 누르는 그 순간이 바로 인증이 일어나는 시점입니다. 비유하자면 병원에 가서 접수를 할 때 신분증을 보여주고 "저는 홍길동입니다"라는 사실을 확인받는 과정과 같습니다. 인증에 성공하면 시스템은 "이 사람은 진짜 홍길동이 맞다"는 사실을 알게 됩니다. 영어 단어 Authentication의 앞부분 Authentic이 "진짜인, 진품인"이라는 뜻을 가지고 있다는 걸 떠올리면 조금 더 기억하기 쉬울 것입니다.

### **인가 (Authorization)**

인가는 인증과 자주 혼동되지만 명확히 다른 개념입니다. 인가는 "이미 신원이 확인된 사람이 특정 행동을 하거나 특정 자원에 접근할 자격이 있는지 확인하는 절차"입니다. 다시 병원 비유를 이어가면, 접수(인증)를 마친 홍길동 씨가 이제 진료실에 들어가려고 하는데, 이 사람이 오늘 진료 예약을 한 사람인지, 혹은 의사인지 환자인지에 따라 들어갈 수 있는 방이 다른 것이 바로 인가입니다. 정리하면 인증은 "너 누구야?"를 확인하는 것이고, 인가는 "너 이거 해도 돼?"를 확인하는 것입니다. 실무에서 흔히 발생하는 401 에러는 인증이 안 된 상태(로그인 자체가 안 되어 있음)를 의미하고, 403 에러는 인증은 되었지만 인가가 거부된 상태(로그인은 했지만 권한이 없음)를 의미한다는 점도 함께 기억해두면 좋습니다.

### **세션 (Session)**

세션은 서버가 특정 사용자에 대한 정보를 임시로 기억해두는 저장 공간입니다. 사용자가 로그인에 성공하면 서버는 "이 사람은 로그인한 상태"라는 정보를 세션이라는 곳에 저장해두고, 이후 이 사용자가 보내는 요청마다 세션을 확인해서 "아, 이 사람은 이미 로그인했었지"라고 판단합니다. 세션은 마치 회사에서 방문자에게 발급하는 임시 출입 기록과 비슷한데, 프론트 데스크(서버)의 대장에 "몇 시에 누가 방문해서 아직 건물 안에 있다"는 기록이 남아있는 것과 같은 원리입니다.

### **쿠키 (Cookie)**

쿠키는 서버가 사용자의 브라우저에게 보내서 저장하게 만드는 작은 데이터 조각입니다. 앞서 설명한 세션과 쿠키는 짝을 이루어 동작하는데, 세션 정보 자체는 서버에 저장되어 있고, 그 세션을 구별할 수 있는 고유번호(세션 ID)만 쿠키에 담겨서 사용자의 브라우저에 저장됩니다. 앞서 든 회사 방문자 비유를 이어가면, 쿠키는 방문자가 목에 거는 임시 출입증 카드라고 볼 수 있습니다. 카드에는 방문자의 모든 정보가 담긴 게 아니라 단순히 방문자 번호만 적혀 있고, 그 번호로 프론트 데스크 대장(세션 저장소)을 조회하면 방문자의 자세한 정보를 알 수 있는 것입니다.

### **필터 체인 (Filter Chain)**

Spring Security는 사용자의 요청이 실제 비즈니스 로직(컨트롤러)에 도달하기 전에 여러 단계의 검문소를 통과시킵니다. 이 검문소들이 순서대로 이어져 있는 구조를 필터 체인이라고 부릅니다. 공항의 보안 검색대를 떠올리면 쉽습니다. 승객(요청)은 신분증 검사대, 짐 검사대, 탑승구 검사대를 순서대로 통과해야 비행기(컨트롤러)에 탑승할 수 있는데, 이 각각의 검사대가 하나의 필터이고, 이 검사대들이 줄지어 있는 전체 과정이 필터 체인입니다.

### **암호화(해싱)와 PasswordEncoder**

비밀번호를 데이터베이스에 저장할 때는 사용자가 입력한 그대로(평문)를 저장하지 않고, 특수한 함수를 거쳐서 알아볼 수 없는 값으로 변환한 뒤 저장합니다. 이 과정을 해싱(Hashing)이라고 부르고, 이 작업을 해주는 도구가 `PasswordEncoder`입니다. 중요한 점은 이 해싱이 "한쪽 방향으로만 가는 문"과 같다는 것입니다. 원본 비밀번호를 해시값으로 바꾸는 건 쉽지만, 해시값을 보고 원본 비밀번호를 거꾸로 알아내는 건 사실상 불가능하게 설계되어 있습니다. 그래서 로그인할 때는 사용자가 입력한 비밀번호를 다시 같은 방식으로 해싱한 뒤, 그 결과가 데이터베이스에 저장된 해시값과 일치하는지를 비교하는 방식으로 검증합니다.

이 여섯 가지 용어, 즉 인증, 인가, 세션, 쿠키, 필터 체인, 암호화(PasswordEncoder)를 머릿속에 넣어두고 아래 본문을 읽으면, 각 코드 조각이 왜 그렇게 작성되었는지가 훨씬 선명하게 보일 것입니다. 이제 이 용어들이 실제로 어떻게 코드와 흐름으로 구현되는지 하나씩 살펴보겠습니다.
...
비전공자를 위해 조금 더 쉽게 비유하면, 유저 관리 기능은 아파트 관리사무소가 입주민 명부를 관리하는 것과 비슷합니다. 누가 몇 호에 사는지(식별자), 그 사람이 집주인인지 세입자인지(권한, 앞서 설명한 인가와 연결되는 개념), 그리고 그 사람이 진짜 그 집에 사는 사람인지 확인하는 절차(비밀번호 확인, 앞서 설명한 인증과 연결되는 개념)가 모두 필요합니다. 이 명부를 아무나 열람하거나 조작할 수 있다면 아파트 전체의 보안이 무너지는 것처럼, 유저 정보를 허술하게 관리하면 서비스 전체의 보안이 무너집니다.

### **1-2. 왜 이 개념을 배우는지**

신입 개발자가 실무에 들어가서 가장 먼저 맡게 되는 업무 중 하나가 회원 관련 기능인 경우가 많습니다. 그 이유는 유저 관리가 거의 모든 서비스의 기초 뼈대이기 때문입니다. 게시판이든 커머스든 결제 시스템이든, "누가 이 요청을 보냈는가"를 알아야 그 다음 로직을 처리할 수 있습니다. 따라서 유저 관리 기능을 제대로 이해하지 못하면 이후에 배우는 인증과 인가도 이해하기 어려워집니다.

또한 이 부분에서 배우는 비밀번호 암호화 개념은 실제 보안 사고와 직결됩니다. 실제로 여러 서비스에서 비밀번호를 평문으로 저장했다가 데이터베이스가 유출되어 사용자 계정이 대량으로 탈취된 사고가 있었습니다. 이런 사고를 방지하기 위해 비밀번호는 절대로 그대로 저장하지 않고, 앞서 용어 정리에서 설명한 단방향 암호화(해싱) 알고리즘을 거쳐서 저장해야 한다는 원칙을 이 단계에서 반드시 익혀야 합니다.
...
Spring Security는 스프링 진영에서 공식으로 제공하는 보안 프레임워크로, 앞서 용어 정리에서 설명한 인증(Authentication)과 인가(Authorization)를 손쉽게 구현할 수 있도록 도와줍니다. 다시 한번 정리하면 인증은 "당신이 누구인지 확인하는 절차"이고, 인가는 "당신이 확인된 사람으로서 이 자원에 접근할 권한이 있는지 확인하는 절차"입니다. 회사 건물에 출입증을 찍는 것은 인증이고, 그 출입증으로 아무 층이나 열 수 있는 게 아니라 자신이 속한 부서의 층만 열리는 것은 인가에 해당한다는 비유를 다시 떠올려보면 좋습니다.

Spring Security의 핵심은 앞서 설명한 필터 체인(Filter Chain)이라는 구조입니다. 사용자의 모든 HTTP 요청은 실제 컨트롤러에 도달하기 전에 여러 개의 필터를 순서대로 통과하게 되는데, 이 필터들이 마치 공항 검색대처럼 하나씩 요청을 검사합니다. 예를 들어 로그인 요청은 `UsernamePasswordAuthenticationFilter`라는 필터가 가로채서 처리하고, 이미 로그인된 사용자의 요청은 세션이나 토큰을 검증하는 필터가 가로채서 "이 사람이 누구인지"를 확인한 뒤 다음 필터로 넘겨줍니다. 이 모든 필터를 통과한 요청만 최종적으로 우리가 작성한 컨트롤러 코드에 도달하게 됩니다.
...
또한 실무에서 협업할 때 Spring Security는 사실상 표준으로 자리잡고 있기 때문에, 이 프레임워크의 동작 원리를 이해하지 못하면 팀에서 작성된 보안 관련 코드를 읽거나 디버깅하는 데 큰 어려움을 겪게 됩니다. 특히 로그인이 실패하는 버그, 권한 문제로 403 에러가 발생하는 문제(앞서 용어 정리에서 설명한 인가 거부 상황) 등을 해결하려면 필터 체인이 어떤 순서로 동작하는지 반드시 이해하고 있어야 합니다.
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
            ...
            .authorizeHttpRequests(auth -> auth
                .requestMatchers("/signup", "/login").permitAll() // 인증 없이 접근 허용
                .requestMatchers("/admin/**").hasRole("ADMIN")    // 인가: ADMIN 권한 필요
...
이 부분을 이해하기 위해서는 먼저 HTTP라는 프로토콜의 근본적인 특징 하나를 알아야 합니다. HTTP는 "무상태(Stateless)" 프로토콜입니다. 이 말은 서버가 클라이언트의 요청을 처리하고 응답을 보내고 나면, 그 요청에 대한 기억을 전혀 남기지 않는다는 뜻입니다. 마치 매번 처음 만나는 사람처럼, 서버는 같은 사용자가 연속으로 요청을 보내더라도 "이 사람이 방금 그 사람이구나"라는 걸 스스로 알 수 없습니다. 하지만 우리가 사용하는 대부분의 서비스는 한 번 로그인(인증)하면 그 상태가 유지되어야 하죠. 이 문제를 해결하기 위해 등장한 것이 앞서 용어 정리에서 설명한 쿠키와 세션입니다.

다시 한번 짧게 요약하면, 쿠키는 서버가 사용자의 브라우저에게 "이 정보를 저장해두고, 다음에 나에게 요청 보낼 때마다 같이 보내줘"라고 넘겨주는 작은 데이터 조각이고, 세션은 서버 측에 저장되는 사용자의 상태 정보입니다. 사용자가 로그인에 성공하면 서버는 그 사용자를 위한 세션을 하나 만들고, 이 세션을 구분할 수 있는 고유한 식별자(세션 ID, 흔히 `JSESSIONID`라는 이름의 쿠키에 담겨 전달됨)를 쿠키에 담아 브라우저에게 돌려줍니다. 이후 브라우저는 같은 서버로 요청을 보낼 때마다 이 쿠키를 자동으로 함께 전송하고, 서버는 쿠키 속 세션 ID를 보고 "아, 이 사람은 아까 로그인했던 그 사용자구나"라고 인식할 수 있게 됩니다.
...
세 가지 이론을 하나로 엮어보면 자연스러운 흐름이 보입니다. 먼저 유저 관리 기능에서 "누구인지 저장하고 확인할 수 있는 데이터 구조"를 만들고, Spring Security 기본기에서 "그 데이터를 안전하고 표준화된 방식으로 검증(인증)하고 접근을 제어(인가)하는 프레임워크"를 익히고, 마지막으로 쿠키/세션 기반 인증에서 "검증이 끝난 사용자의 상태를 어떻게 유지시키는지"를 배우는 것입니다. 이 세 가지를 하나의 흐름으로 이해하면, 회원가입부터 로그인, 그리고 로그인 이후 페이지 접근까지 전체 사이클이 어떻게 맞물려 돌아가는지 훨씬 선명하게 그려질 것입니다. 초급 개발자 입장에서는 처음엔 인증, 인가, 필터, 매니저, 세션 저장소 같은 용어들이 낯설게 느껴질 수 있지만, 결국 이 모든 구성 요소는 "이 사람이 누구인지 확인하고(인증), 그 사람이 무엇을 할 수 있는지 판단하고(인가), 확인된 상태를 안전하게 유지한다"는 세 가지 목적을 위해 움직이고 있다는 점을 기억하면 개념을 훨씬 수월하게 붙잡을 수 있습니다.
